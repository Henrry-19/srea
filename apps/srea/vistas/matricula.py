...
class MatriculaListView(LoginRequiredMixin,ValidatePermissionRequiredMixin,ListView): #Primera vista basada en clase ListView, permite sobrescribir métodos
    model= Matricula#Primero se indica el modelo o entidad
    template_name = 'matricula/matricula_lista.html' #Indicarle cual es la plantilla
    ...
    def get_context_data(self, **kwargs): #Método que devuelve un diccionario que representa el contexto de la plantilla
        ...
        context['title']='Listado de matriculas' #Puedo enviar variables
        context['url_create']=reverse_lazy('srea:r_matricula')#Ruta abosluta creación de matrícula
        #context['url_list']=reverse_lazy('srea:p_matricula')#Ruta abosluta lista de usuario
        context['modelo']='Matricula'#Nombre de identidad
        return context


class MatriculaCreateView(LoginRequiredMixin,ValidatePermissionRequiredMixin,CreateView):
    model=Matricula #Indicar el modelo con el cual se va ha trabajar
    form_class=MatriculaCreateForm #Importando el formulario con el que voy a trabajar
    template_name='matricula/matricula_create.html' # Debo indicarle la ubicación de mi plantilla
    permission_required='add_matricula'
    success_url= reverse_lazy('srea:matricula') #Me permite direccionar a otra plantilla, la funnción reverse_lazy me recibe una url como parámetro
    ...
    def get_context_data(self, **kwargs): #Método que devuelve un diccionario que representa el contexto de la plantilla
        context = super().get_context_data(**kwargs) #Obtengo el diccionario que devuelve el método
        context['title']='Creación de una matricula' #Puedo enviar variables
        context['modelo']='Matricula'#Nombre de identidad
        context['url_list']=reverse_lazy('srea:matricula')#Ruta abosluta lista de asignatura
...
class MatriculaUpdateView(LoginRequiredMixin,ValidatePermissionRequiredMixin,UpdateView):
    ...
    success_url = reverse_lazy('srea:matricula') #Me permite direccionar a otra plantilla, la función reverse_lazy me recibe una url como parámetro
    #@method_decorator(csrf_exempt) #Mecanismo de defensa de django
    ...
    def get_context_data(self, **kwargs): #Método que devuelve un diccionario que representa el contexto de la plantilla
        context = super().get_context_data(**kwargs) #Obtengo el diccionario que devuelve el método
        context['title']='Actualización de una matricula' #Puedo enviar variables
        context['modelo']='Matricula'#Nombre de identidad
        context['url_list']=reverse_lazy('srea:matricula')#Ruta abosluta lista de asignatura
...
class MatriculaDeleteView(LoginRequiredMixin,ValidatePermissionRequiredMixin,DeleteView):
    model =Matricula #Indicar el modelo con el cual se va ha trabajar
    template_name = 'matricula/matricula_delete.html' #Debo indicarle la ubicación de mi plantilla
    success_url= reverse_lazy('srea:matricula')#Me permite direccionar a otra plantilla, la función reverse_lazy me recibe una url como parámetro
    permission_required='delete_matricula'
