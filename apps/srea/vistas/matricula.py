...
class MatriculaListView(LoginRequiredMixin,ValidatePermissionRequiredMixin,ListView): #Primera vista basada en clase ListView, permite sobrescribir métodos
    ...
    template_name = '../templates/matricula/evaluarEA.html' #Indicarle cual es la plantilla
    ...
    def post(self, request, *args, **kwargs):###Implementación de ajax en mi método sobrescrito POST###
        ...
        try: #controlar el error
            ...
            if action == 'searchdata':
                data=[]
                position = 1
                for i in Matricula.objects.filter(id=self.id):
                    item= i.toJSON()
    ...
    def get_context_data(self, **kwargs): #Método que devuelve un diccionario que representa el contexto de la plantilla
        ...
        context['modelo']='Matriculas'#Nombre de identidad
        return context
