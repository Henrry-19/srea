...
############################################################################
class CatalogListView(LoginRequiredMixin,ValidatePermissionRequiredMixin,View):
    model= Catalog
    template_name = 'catalogo/catalogo_lista.html'
    permission_required='view_catalog'
    def get(self,request, *args, **kwargs):
        #user=request.user
        if request.user.is_staff : 
            catalogo = Catalog.objects.all()
        if  not request.user.is_staff:
           catalogo = Catalog.objects.all()
        #print(catalogo_item)
        context={
            'catalogo':catalogo,
            'url_create':reverse_lazy('srea:catalogo_create'),
            'list_url':reverse_lazy('srea:catalogo'),
            'title':'Lista de Catálogo',
            'modelo':'Lista de Catálogo',
            'date_now':datetime.now()  
        }

        return render(request, 'catalogo/catalogo_lista.html', context)
    ...
    def get_context_data(self, **kwargs): #Método que devuelve un diccionario que representa el contexto de la plantilla
        ...
        #context['url_create']=reverse_lazy('srea:catalogo_create')#Ruta abosluta creación de usuario
        #context['url_list']=reverse_lazy('srea:catalogo')#Ruta abosluta lista de usuario
        #context['modelo']='Asignaturas'#Nombre de identidad
        return context

class CatalogoCreateView(LoginRequiredMixin,ValidatePermissionRequiredMixin,CreateView):
    model=Catalog #Indicar el modelo con el cual se va ha trabajar
    form_class=CatalogoCreateForm #Importando el formulario con el que voy a trabajar
    template_name='catalogo/catalogo_create.html' # Debo indicarle la ubicación de mi plantilla
    ...
    permission_required='add_catalog'
    ...
    def get_context_data(self, **kwargs): #Método que devuelve un diccionario que representa el contexto de la plantilla
        context = super().get_context_data(**kwargs) #Obtengo el diccionario que devuelve el método
        context['title']='Creación de un Catálogo' #Puedo enviar variables
        context['modelo']='Catálogo'#Nombre de identidad
        context['url_list']=reverse_lazy('srea:catalogo')#Ruta abosluta lista de asignatura
        context['action']='add'#Enviar variable action
        return context
    
class CatalogoUpdateView(LoginRequiredMixin,ValidatePermissionRequiredMixin,UpdateView):
    model = Catalog #Indicar el modelo con el cual se va ha trabajar
    form_class = CatalogoCreateForm #Importando el formulario con el que voy a trabajar
    template_name = 'catalogo/catalogo_create.html' #Debo indicarle la ubicación de mi plantilla
    permission_required='change_catalog'
    ...
    def get_context_data(self, **kwargs): #Método que devuelve un diccionario que representa el contexto de la plantilla
        context = super().get_context_data(**kwargs) #Obtengo el diccionario que devuelve el método
        context['title']='Actualización de un Catálogo' #Puedo enviar variables
        context['modelo']='Catálogo'#Nombre de identidad
        context['url_list']=reverse_lazy('srea:catalogo')#Ruta abosluta lista de asignatura
        context['action']='edit'#Enviar variable action
        return context

class CatalogoDeleteView(LoginRequiredMixin,ValidatePermissionRequiredMixin,DeleteView):
    model = Catalog #Indicar el modelo con el cual se va ha trabajar
    template_name = 'catalogo/catalogo_delete.html' #Debo indicarle la ubicación de mi plantilla
    success_url= reverse_lazy('srea:catalogo')#Me permite direccionar a otra plantilla, la función reverse_lazy me recibe una url como parámetro
    permission_required='delete_catalog'
    ...
    def get_context_data(self, **kwargs):
        ...
        context['title'] = 'Eliminación de un catálogo'
        context['modelo'] = 'Catálogo'
        context['url_list'] = reverse_lazy('srea:catalogo')
        return context
