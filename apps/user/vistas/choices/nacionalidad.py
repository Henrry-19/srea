nacionalidad_pais=(
    ('Afgana', 'Afganistán'),
    ('Albanés', 'Albania'),
    ('Alemana', 'Alemania'),
    ('Andorrana', 'Andorra'),
    ('Angoleña', 'Angola'),
    ('Antiguana', 'Antigua y Barbuda'),
    ('Árabe', 'Arabia Saudita / Arabia Saudí'),
    ('Argelina', 'Argelia'),
    ('Argentina', 'Argentina'),
    ('Armenia', 'Armenia'),
    ('Australiana ', 'Australia'),
    ('Austriaca', 'Austria'),
    ('Azerbaiyano', 'Azerbaiyán'),
    ('Bahamas', 'Bahamas'),
    ('Bangladesí', 'Bangladés'),
    ('Barbadense', 'Barbados'),
    ('Bareiní', 'Baréin'),
    ('Belga', 'Bélgica'),
    ('Beliceña', 'Belice'),
    ('Bielorusa', 'Bielorrusia'),
    ('Beninesa', 'Benín'),
    ('Birmana', 'Birmania'),
    ('Boliviana ', 'Bolivia'),
    ('Bosnia', 'Bosnia y Herzegovina'),
    ('Botsuana', 'Botsuana'),
    ('Brasileña', 'Brasil'),
    ('Bruneana', 'Brunei'),
    ('Búlgara', 'Bulgaria'),
    ('Burkinés', 'Burkina Faso'),
    ('Burundesa', 'Burundi'),
    ('Butanés', 'Bután'),
    ('Caboverdiano', 'Cabo Verde'),
    ('Camboyana', 'Camboya'),
    ('Camerunesa', 'Camerún'),
    ('Canadiense', 'Canadá'),
    ('Centroafricana', 'República Centroafricana'),
    ('Chadiana', 'Chad'),
    ('Checa', 'República Checa / Chequia'),
    ('Chilena', 'Chile'),
    ('China', 'China'),
    ('Chipriota', 'Chipre'),
    ('Colombiana ', 'Colombia'),
    ('Comoras', 'Comoras'),
    ('Congoleña', 'República del Congo'),
    ('Congoleña,', 'República Democrática del Congo'),
    ('Coreana', 'Corea del Norte'),
    ('Coreana', 'Corea del Sur'),
    ('Costarricense', 'Costa de Marfil'),
    ('Costarricense', 'Costa Rica'),
    ('Croata', 'Croacia'),
    ('Cubana', 'Cuba'),
    ('Danesa', 'Dinamarca'),
    ('Dominiquesa', 'Dominica'),
    ('Dominicana', 'República Dominicana'),
    ('Ecuatoriana', 'Ecuador'),
    ('Egipcia', 'Egipto'),
    ('Salvadoreña', 'El Salvador'),
    ('Emiratí', 'Emiratos Árabes Unidos'),
    ('Eritrea', 'Eritrea'),
    ('Eslovaca', 'Eslovaquia'),
    ('Eslovena ', 'Eslovenia'),
    ('Española', 'España'),
    ('Estadounidense', 'Estados Unidos'),
    ('Estona', 'Estonia'),
    ('Etiope', 'Etiopía'),
    ('Filipina', 'Filipinas'),
    ('Finlandesa', 'Finlandia'),
    ('Fiyiana', 'Fiyi'),
    ('Francesa', 'Francia'),
    ('Gabonesa', 'Gabón'),
    ('Gambiana', 'Gambia'),
    ('Georgiana', 'Georgia'),
    ('Ghanesa', 'Ghana'),
    ('Granadina', 'Granada'),
    ('Griega', 'Grecia'),
    ('Guatemalteca', 'Guatemala'),
    ('Guineana', 'Guinea'),
    ('Bisauguineana', 'Guinea-Bisáu'),
    ('Ecuatoguineana', 'Guinea Ecuatorial'),
    ('Guyanesa', 'Guyana'),
    ('Haitiana ', 'Haití'),
    ('Hondureña', 'Honduras'),
    ('Húngara', 'Hungría'),
    ('Indonesa ', 'India'),
    ('Indonesa', 'Indonesia'),
    ('Iraquí ', 'Irak'),
    ('Iraní', 'Irán'),
    ('Irlandesa', 'Irlanda'),
    ('Islandesa', 'Islandia'),
    ('Israelí', 'Israel'),
    ('Italiana', 'Italia'),
    ('Jamaicana', 'Jamaica'),
    ('Japonesa ', 'Japón'),
    ('Jordana', 'Jordania'),
    ('Kazaj0', 'Kazajistán'),
    ('Keniana', 'Kenia'),
    ('Kirguisa', 'Kirguistán'),
    ('Kiribatiana', 'Kiribati'),
    ('Kuwaití', 'Kuwait'),
    ('Laosiana', 'Laos'),
    ('Lesotense', 'Lesoto'),
    ('Letona', 'Letonia'),
    ('Libanesa', 'Líbano'),
    ('Liberiana', 'Liberia'),
    ('Libia', 'Libia'),
    ('Liechtensteiniano', 'Liechtenstein'),
    ('Lituana', 'Lituania'),
    ('Luxemburguesa', 'Luxemburgo'),
    ('Macedonia', 'Macedonia del Norte'),
    ('Malgache', 'Madagascar'),
    ('Malaya ', 'Malasia'),
    ('Malauí', 'Malaui'),
    ('Maldiva', 'Maldivas'),
    ('Malies', 'Mali / Malí'),
    ('maltesa', 'Malta'),
    ('Marroquí', 'Marruecos'),
    ('Marshalesa', 'Islas Marshall'),
    ('Mauriciana', 'Mauricio'),
    ('Mauritana', 'Mauritania'),
    ('Mexicana', 'México'),
    ('Micronesio', 'Micronesia'),
    ('Moldava', 'Moldavia'),
    ('Monegasca', 'Mónaco'),
    ('Mongola', 'Mongolia'),
    ('Montenegrina', 'Montenegro'),
    ('Mozambiqueña', 'Mozambique'),
    ('Namibia', 'Namibia'),
    ('Nauruano', 'Nauru'),
    ('Nepalí', 'Nepal'),
    ('Nicaragüense ', 'Nicaragua'),
    ('Nigerina', 'Níger'),
    ('Nigeriana', 'Nigeria'),
    ('Noruega', 'Noruega'),
    ('Neozelandesa', 'Nueva Zelanda / Nueva Zelandia'),
    ('Omaní', 'Omán'),
    ('Holandesa', 'Países Bajos'),
    ('Pakistaní', 'Pakistán'),
    ('Palauana', 'Palaos'),
    ('Palestina', 'Palestina'),
    ('Panameña', 'Panamá'),
    ('Papuanos', 'Papúa Nueva Guinea'),
    ('Paraguaya', 'Paraguay'),
    ('Peruana', 'Perú'),
    ('Polaca', 'Polonia'),
    ('Portuguesa 	', 'Portugal'),
    ('Qatarí', 'Qatar'),
    ('Británica', 'Reino Unido'),
    ('Ruandesa', 'Ruanda'),
    ('Rumana', 'Rumania / Rumanía'),
    ('Rusa', 'Rusia'),
    ('Salomonense', 'Islas Salomón'),
    ('Samoana', 'Samoa'),
    ('Sancristobaleña', 'San Cristóbal y Nieves'),
    ('Sanmarinense', 'San Marino'),
    ('Sanvicentina', 'San Vicente y las Granadinas'),
    ('Santalucense', 'Santa Lucía'),
    ('Santotomense', 'Santo Tomé y Príncipe'),
    ('Senegalesa', 'Senegal'),
    ('Serbia', 'Serbia'),
    ('Seychelense', 'Seychelles'),
    ('Sierraleonés', 'Sierra Leona'),
    ('Singapurense', 'Singapur'),
    ('Siria', 'Siria'),
    ('Somalia', 'Somalia'),
    ('Esrilanqués', 'Sri Lanka'),
    ('Suazi', 'Suazilandia / Esuatini'),
    ('Sudafricana', 'Sudáfrica'),
    ('Sudanesa', 'Sudán'),
    ('Sursudanesa', 'Sudán del Sur'),
    ('Sueca', 'Suecia'),
    ('Suiza', 'Suiza'),
    ('Surinamesa', 'Surinam'),
    ('Tailandesa', 'Tailandia'),
    ('Tanzana', 'Tanzania'),
    ('Tayika', 'Tayikistán'),
    ('Timorense', 'Timor Oriental'),
    ('Togolesa', 'Togo'),
    ('Tongana', 'Tonga'),
    ('Trinitense', 'Trinidad y Tobago'),
    ('Tunecina', 'Túnez'),
    ('Turcomana', 'Turkmenistán'),
    ('Turca', 'Turquía'),
    ('Tuvaluana', 'Tuvalu'),
    ('Ucraniana', 'Ucrania'),
    ('Ugandesa', 'Uganda'),
    ('Uruguaya', 'Uruguay'),
    ('Uzbeka', 'Uzbekistán'),
    ('Vanuatuense', 'Vanuatu'),
    ('Vaticana', 'Ciudad del Vaticano'),
    ('Venezolana', 'Venezuela'),
    ('Vietnamita', 'Vietnam'),
    ('Yemenita', 'Yemen'),
    ('Yibutiana', 'Yibuti'),
    ('Zambiana', 'Zambia'),
    ('Zimbabuense', 'Zimbabue'),
    ('Otro', 'Otro')
)